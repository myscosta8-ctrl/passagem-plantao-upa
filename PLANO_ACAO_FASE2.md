...
| Sidebar redesenhada | ✅ Feito (commit `7a6336a`), fidelidade visual corrigida (commit `ad5f6cf`, 25/09) | A estrutura (grupos, colapsar/expandir, hambúrguer mobile) já existia desde antes; emoji trocados por Phosphor. **Correção 25/09:** usuário identificou que a lista de itens do menu diverge entre os 3 mockups existentes (`proposta-menu-hamburguer.html`, `01-painel-leitos-design.html`, `12-passagem-plantao-design.html`) — por decisão do usuário, `12-passagem-plantao-design.html` passou a ser a referência oficial **só de design visual** (cores, espaçamento, estado ativo), mantendo a lista de itens/funções já existente no app (que não segue nenhum mockup específico, por serem inconsistentes entre si). `Sidebar.css` restilizado: fundo `#0F172A`, item ativo com tom teal translúcido e texto `#2DD4BF` (era bloco azul sólido), badges com raio 4px. |
...
| 01 — Painel de Leitos | ✅ Concluído (commit `8148de5`), fidelidade corrigida (commit `ad5f6cf`, 25/09) | `Painel.css` já batia com o mockup (cores, risk-bar Manchester, grid de cards); faltavam só os ícones — trocados emoji/símbolos unicode por Phosphor em `Painel.jsx`, `PainelCards.jsx`, `ModalInternar.jsx`. **Correção 25/09:** usuário apontou que faltava a busca de paciente/leito + botão "Filtrar" no cabeçalho (mockup `01-painel-leitos-design.html`) — antes só existiam dentro do modo Lista. Extraído `PainelControles.jsx` com busca+filtro (Setor/Status) compartilhados pelos dois modos; `PainelCards.jsx` passou a aplicar os mesmos filtros que já existiam só na Lista. Nenhuma funcionalidade removida, só completada. |
...
| 12 — Passagem de Plantão coletiva | ✅ Primeiro incremento (commit `e67bc7e`), navegação corrigida (commit `8497815`) | Usuário escolheu explicitamente construir a grade coletiva nova (não só reskin do formulário existente). Componente `PassagemColetiva.jsx` (presentacional, reusa dados já carregados por `usePainelState`, sem duplicar busca). Inclui: pills de setor, chips de filtro (todos/pendências/a conferir/conferidos), cards com dados reais reaproveitando `IndicadoresClinicos`, toggle "Conferido" (carimbo de revisão reversível, novas colunas `conferido_por`/`conferido_em` em `passagens`, migração aditiva já aplicada em produção) e botão "Detalhes" que abre o `PassagemForm.jsx`/`EspacoPaciente` existente sem nenhuma alteração. **Nenhum dado de produção foi alterado ou apagado** — só colunas novas, nullable. **Correção de navegação (25/09):** o usuário identificou, com evidência do mockup 12, que "Passagem de Plantão" deveria ser item próprio do menu lateral (grupo Assistencial & Leitos, badge "Setor"), não um 3º modo de visualização dentro do Painel de Leitos — corrigido: nova tela de topo `passagemColetiva` (`PassagemColetivaTela.jsx`), item dedicado na Sidebar, removido o toggle de dentro do Painel. **Card reconstruído (commit `00e2f84`, 25/09):** o usuário apontou que o card não batia com o mockup — reconstruído como lista vertical de cards largos com cabeçalho (recolher/expandir, alergia com substância real, botões Conferir/Editar Passagem/Prontuários) e 4 colunas de conteúdo (Sinais Vitais reais, Dispositivos & Cuidados, Balanço Hídrico real, Pendências editável direto na grade com chips de atalho genéricos). Fora do escopo, mantido: botão "Sincronizar Sinais & Balanço" (recarregar já cobre), modo tabela/foco, painel lateral com histórico de plantões. |
| 09 — Evolução de Enfermagem (SAE) | ✅ Concluído (commit `707a64a`), NANDA-I/NIC adicionados (commit `d685550`, 25/09) | `AbaEvolucao.jsx` reescrito no padrão split-view. **25/09:** migração aditiva em `evolucoes` (`diagnosticos_nanda`, `prescricao_nic`, ambos `text[]`) e catálogos completos do mockup (8 diagnósticos NANDA-I, 6 cuidados NIC com frequência fixa) implementados como checkboxes reais, nenhum marcado por padrão (diferente do mockup, que vem pré-selecionado como exemplo). Fora do escopo, mantido: grade de sinais vitais do turno (já existe em aba própria, `AbaSinaisVitais`, evitando duplicar entrada do mesmo dado). |
| 13 — Nota de Intercorrência de Enfermagem | ✅ Concluído (commit `7ec2bf0`), campos extras adicionados (commit `53ae57e`, 25/09) | `AbaEventosAdversos.jsx` reescrito no padrão split-view (timeline + `clinical-card`). Categoria e gravidade viraram radio real (seleção única). **25/09:** migração aditiva em `eventos_adversos` (`medico_comunicado`, `horario_comunicacao_medico`, `sv_pa_sistolica/sv_pa_diastolica/sv_fc/sv_fr/sv_temperatura/sv_spo2`, `desfecho_evolucao`) e UI completada: horário exato da ocorrência editável (`ocorrido_em` já existia no schema, só não era exposto), checkbox de médico comunicado + horário, grade de sinais vitais no momento do evento, campo de desfecho/evolução pós-conduta. |
| 16 — Solicitação de Exames & APAC | ✅ Concluído (commit `1ff8a4d`) — decisão de arquitetura confirmada (25/09) | `AbaExames.jsx` (lab/img/ecg) e `AbaApac.jsx` (laudo APAC, 52 campos SUS) reescritos com o framework compartilhado, ícones Phosphor. Catálogos de exames, protocolos rápidos e toda a lógica de salvar/imprimir preservados verbatim. **O mockup 16 mescla as 4 modalidades numa única tela — decisão final: NÃO adotar.** Confirmado pelo usuário em 25/09: a APAC é uma solicitação de regulação **externa** (uso fora da UPA), enquanto Laboratório/Imagem/ECG são exames **internos** — misturar as duas numa tela só confundiria o fluxo assistencial. A separação em duas abas (Exames vs APAC) é a arquitetura correta e permanece assim; não é uma pendência, é a decisão definitiva. |
| 11 — Balanço Hídrico 24h | ✅ Concluído (commit `8e09671`), grade editável implementada (commit `e5edd41`, 25/09) | `AbaBalancoHidrico.jsx` reescrito com o framework compartilhado, cards de métrica cumulativa (entradas/saídas/balanço). **25/09:** substituído o formulário único por duas tabelas lado a lado (Entradas/Saídas), aproximando da grade do mockup — sem nenhuma alteração de schema (`via` existente vira coluna Via/Tipo, `observacao` vira coluna Item/Aspecto). Deliberadamente fora do escopo: horário editável arbitrário por célula (o sistema usa o timestamp real do servidor, não permite retroagir sem função de update que não existe) e subtotais "por turno" (Manhã/Tarde/Noite) — o componente não recebe informação de turno do plantão. |
| 14 — Transferência Estruturada (SBAR) | ✅ Concluído (commit `ee52d15`), pilar B adicionado (commit `b845577`, 25/09) | `AbaSbar.jsx` reescrito no padrão split-view, pilares S/A/R com cores de identificação. **25/09:** adicionado pilar B (Breve Histórico) — campo livre de antecedentes/condutas já realizadas na UPA, vazio por padrão. Não precisou de migração: coluna `breve_historico` já existia na tabela (órfã, nunca exposta). Fora do escopo, mantido deliberadamente: checklist de pertences do paciente e "Avaliação Neurológica"/"Acessos e Drogas em Infusão" detalhados — o mockup preenche esses campos com texto clínico fabricado como exemplo. |
| 10 — Checagem/Aprazamento (Cardex) | **Excluído do escopo a pedido do usuário** | — |
| — (sem número) "1. Admissão" (`AbaAdmissao.jsx`, exame físico 26 grupos) | ✅ Reskinado (commit `a4938b3`), **removido (commit `e6f3155`, 25/09)** | Módulo órfão descoberto durante o item 5 — distinto de "2. Admissão de Enfermagem" (módulo 08), sem mockup próprio na Fase 2. Reskinado primeiro; depois, a pedido explícito do usuário, removido do Prontuário de Enfermagem por ser redundante com "Admissão de Enfermagem" (SAE, fiel ao mockup 08), que passou a ser a única entrada e a aba padrão ao abrir o prontuário. Tabela `admissoes_enfermagem` no banco (1 registro) não foi tocada — só deixou de ter tela dedicada. |
...
|---|---|---|---|---|
| 1 | Evolução Médica Diária | ✅ Concluído (commit `988ab4d`) — Grade de sinais vitais com botão "Puxar da Enfermagem" | Migração aditiva aplicada em produção (colunas nullable `sv_*` em `evolucoes_medicas`). Botão busca o último registro de `sinais_vitais` da Enfermagem via `listarSinaisVitais` (já existente) e preenche os campos, que continuam totalmente editáveis antes de salvar. | Não |
| 2 | Prescrição Médica | ✅ Concluído (commit `5cd0192`) — Calculadora de dose pediátrica EV/IM (diluição, mg/kg, volume a aspirar) | Não precisou de tabela nova — resultado é aplicado nos campos já existentes do item de prescrição (`dose`/`dose_unidade`/`diluicao`). Peso do paciente e demais parâmetros são digitados manualmente a cada uso, pois não há peso armazenado no schema hoje. | Não |
| 3 | Receituário/Sumário de Alta | ✅ Concluído (commit `7838564`) — Classificação automática de controlado/antimicrobiano no Receituário (usando `catalogo_medicamentos.controlado/antimicrobiano` e `receitas_medicas.tipo`, já existentes) + sub-abas de filtro no histórico + Atestado Médico como documento novo (tabela `atestados_medicos`, aba própria, impressão oficial) | Migração aditiva aplicada em produção para `atestados_medicos`. Não foi feita a unificação estrutural de Receituário/Controle Especial/Atestado/Sumário em uma única tela com sub-abas do mockup — cada documento continua em sua própria aba, mudança estrutural maior que reskin/funcionalidade pontual. | Não |
| 4 | Plano Terapêutico | ✅ Concluído (commit `37d6789`) — Kits de protocolo institucional (Sepse/Choque Séptico, Síndrome Coronariana Aguda, AVC) | Redesenhado deliberadamente para não repetir o bug de dado fabricado: os kits marcam apenas protocolos elegíveis e equipe multidisciplinar (opções existentes), nunca preenchem diagnóstico/motivo/objetivos/metas com texto clínico fabricado como o mockup original fazia. | Resolvido — risco eliminado pelo redesenho |
| 5 | Admissão de Enfermagem | ✅ Concluído (commit `8089cd0`) — Sub-abas com filtro de seção, botão expandir/recolher todas as seções, painel lateral fixo com Alergias/Escalas de Risco/Dispositivos | Painel lateral é somente leitura, puxado via `listarAlergias`/`listarEscalas`/`listarDispositivos` (já existentes, mesmas fontes das abas próprias) — sem duplicar cadastro. Nenhum campo/lógica de salvar alterado. | Não |
| 6 | Solicitação de Hemoterapia | ✅ Concluído (commit `d5ed430`) — Protocolos transfusionais (Anemia Aguda Grave, Choque Hemorrágico/Trauma, Plaquetopenia Severa) | Redesenhado deliberadamente para não repetir o bug de dado fabricado: os protocolos marcam apenas hemocomponente/quantidade (opção existente) e urgência, nunca a indicação clínica (texto livre) nem a autorização de extrema urgência (decisão legal, sempre manual). Painel lateral com hemograma "puxado" de Exames continua fora do escopo — cruzamento de dados não mapeado hoje. | Resolvido — risco eliminado pelo redesenho |
| 7 | Recepção (transversal) | ✅ Concluído (commit `dd7a189`) — Redesenho do padrão `.form-field`/`.form-grid`/`.toggle-group`/`.chip-group` em `PassagemForm.css` | Puramente CSS: fonte Inter, campos com contorno completo e cantos arredondados, foco com anel de destaque, chips em pílula. Nenhuma classe/estrutura/lógica JS alterada — beneficia de uma vez os 18 arquivos que ainda usam esse padrão (Recepção, APAC, Regulação, TFD, ATM, Medicações Contínuas, Nota de Intercorrência Médica, e as abas de Admissão/Alergias/Dispositivos/Escalas/Isolamento/Sinais Vitais de enfermagem). | Não |
