...
| — (sem número) "1. Admissão" (`AbaAdmissao.jsx`, exame físico 26 grupos) | ✅ Concluído (commit `a4938b3`) | Módulo órfão descoberto durante o item 5 — distinto de "2. Admissão de Enfermagem" (módulo 08), sem mockup próprio na Fase 2 e não rastreado até então. Reskin com framework compartilhado nas seções de identificação/sinais vitais/integridade física/antecedentes/observações. O acordeão de 26 grupos de exame físico (classes próprias em `FichaClinica.css`) foi preservado — já é uma UI especializada funcional, fora do escopo do redesenho transversal do item 7. |
